...
class VAE():
    ...
    def _create_network(self):
        ...
        self.eps = {
            'eps':
            tf.random_normal([self.batch_size, self.latent_size],
                             stddev=self.stddev,
                             mean=self.mean)
        }

        # Decoder linear layer
        self.weights['softmax'] = tf.get_variable(
            "softmaxw",
        ...
        self.weights['out_mean'] = tf.get_variable(
            ...
            shape=[self.unit_size, self.latent_size]),
        ...
        self.embedding_encode = tf.get_variable(
            name='encode_embedding',
            shape=[self.latent_size, self.vocab_size],
            initializer=tf.random_uniform_initializer(minval=-0.1, maxval=0.1))
        self.embedding_decode = tf.get_variable(
        ...
        self.reconstr_loss = tf.reduce_mean(
            tf.contrib.seq2seq.sequence_loss(logits=decoded_logits,
                                             targets=self.Y,
                                             weights=weights))
    ...
    def encode(self):
        ...
        _, state = tf.nn.dynamic_rnn(self.encoder,
                                     X,
                                     dtype=tf.float32,
                                     scope='encode',
                                     sequence_length=self.L)
        c, h = state[-1]
        self.weights['out_mean'] = tf.reshape(self.weights['out_mean'],
                                              [self.unit_size, -1])
    ...
    def decode(self, Z):
        ...
        self.initial_decoded_state = tuple([
            tf.contrib.rnn.LSTMStateTuple(
                ...
                tf.zeros((self.batch_size, self.unit_size))) for i in range(3)
        ])
        ...
        Y, self.output_decoded_state = tf.nn.dynamic_rnn(
            ...
            scope='decode',
            sequence_length=self.L,
            initial_state=self.initial_decoded_state)
    ...
    def get_latent_vector(self, x, l):
        return self.sess.run(self.latent_vector,
                             feed_dict={
                                 self.X: x,
                                 self.L: l
                             })
    ...
    def cal_latent_loss(self, mean, log_sigma):
        latent_loss = tf.reduce_mean(
            -0.5 * (1 + log_sigma - tf.square(mean) - tf.exp(log_sigma)))
        return latent_loss

    def train(self, x, y, l):
        _, r_loss, l_loss = self.sess.run(
    ...
    def test(self, x, y, l):
        mol_pred, r_loss, l_loss = self.sess.run(
            [self.mol_pred, self.reconstr_loss, self.latent_loss],
            feed_dict={
                self.X: x,
                self.Y: y,
                self.L: l
    ...
    def sample(self, latent_vector, start_codon, seq_length):
        ...
        for i in range(seq_length):
            if i == 0:
                x, state = self.sess.run(
                    [self.mol_pred, self.output_decoded_state],
                    feed_dict={
                        self.X: x,
                ...
                x, state = self.sess.run(
                    ...
                    feed_dict={
                        ...
                        self.latent_vector: latent_vector,
                        self.L: l,
                        self.initial_decoded_state: state
                    })
            preds.append(x)
